...
DownCleaner는 사용 목적에 따라 스캔과 추천 기준을 조정하는 정리 프로필을 제공합니다.

| 프로필 | 대상 | 동작 |
| --- | --- | --- |
| `일상 정리` | 다운로드 폴더와 바탕화면을 방치한 일반 사용자 | 오래된 설치 파일, 압축 파일, 임시 다운로드 파일을 더 잘 찾습니다. 프로젝트/문서 파일은 보호합니다. |
| `대용량 정리` | 저장공간 확보가 우선인 사용자 | 오래된 대용량 압축/영상/설치 파일을 더 강하게 후보로 봅니다. |
| `고급 정리` | 개발, 3D, 디자인 등 작업 폴더를 다루는 사용자 | `node_modules`, `bin`, `obj`, `dist`, Unity `Library` 같은 생성/캐시 폴더까지 분석합니다. 단 `.git`은 보호를 위해 제외합니다. |
| `안전 검토` | 삭제가 조심스러운 사용자 또는 공용/회사 PC | 자동 선택을 사용하지 않고 사용자가 직접 검토하도록 합니다. |

프로필은 `models/ScanPolicy.cs`에서 정책으로 변환되며, `FileScanner.ScanFilesAsync()`와 자동 선택 로직이 이 정책을 사용합니다.
